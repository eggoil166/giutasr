...
async function generateChartFromMP3(filePath, filename) {
  try {
    ...
    // Analyze the MP3 file with Gemini AI
    const prompt = `Analyze this MP3 file "${filename}" and generate a guitar chart for a rhythm game. 
    Please provide:
    1. Song metadata (name, artist, estimated genre, tempo/BPM)
    2. Musical structure (intro, verse, chorus, bridge, outro sections with timestamps)
    3. Difficulty assessment (1-5 scale)
    4. Chord progression suggestions
    5. Note pattern recommendations for guitar gameplay
    
    Format your response as a structured analysis that I can use to create a guitar chart.`;
